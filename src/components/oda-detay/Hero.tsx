...
export default function RoomHero({
  slug = "3-kisilik-deluxe-oda",
  roomTitle,
  capacity = "3 Kişilik",
  size = "28 m²",
  price = "Uygun Fiyat Garantisi",
}: RoomHeroProps) {
  // Slug'dan okunabilir oda adı türetme (Props verilmezse otomatik biçimlendirir)
  ...
  return (
    <section className="relative flex min-h-[80vh] w-full items-center justify-center overflow-hidden bg-slate-950 text-white pt-24 pb-16 lg:pt-32 lg:pb-20">
      ...
      {/* Mavi Ambient Glow / Parlama Efekti */}
      <div 
        aria-hidden="true" 
      ...
      {/* İnce Izgara Desen Katmanı (Minimalist Doku) */}
      <div 
        aria-hidden="true" 
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-lg">
          Öğrencilerimiz için özel olarak tasarlanmış, ergonomik çalışma alanı, geniş depolama imkânı ve yüksek konfor sunan modern yaşam alanı.
        </p>

        {/* Minimalist Cam Özellik Rozetleri */}
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Link
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center rounded-full bg-blue-600 px-8 py-4 text-xs font-bold uppercase tracking-wider text-white shadow-lg shadow-blue-600/25 transition-all duration-300 hover:bg-blue-500 hover:shadow-blue-500/40 active:scale-[0.98]"
          >
            Bu Oda İçin Başvuru Yap →
          </Link>

          <Link
            href={`https://wa.me/905382123011?text=Merhaba,%20${encodeURIComponent(formattedTitle)}%20hakkında%20bilgi%20almak%20istiyorum.`}
            target="_blank"
            rel="noreferrer"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full border border-slate-800 bg-slate-900/80 px-8 py-4 text-xs font-bold uppercase tracking-wider text-slate-200 transition-all duration-300 hover:border-emerald-500/50 hover:bg-slate-800 hover:text-emerald-400 active:scale-[0.98]"
          >
