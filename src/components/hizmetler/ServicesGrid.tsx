  ...
  return (
    <section className="bg-slate-50 py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Mavi Vurgulu Kurumsal Grid Yapısı */}
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {serviceDetails.map((item, index) => (
            <Reveal key={item.title} delay={index * 0.05}>
              <article className="group relative flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-blue-300 hover:shadow-xl hover:shadow-blue-900/5">
                
                {/* Sol Kenar Mavi Accent Çizgisi */}
                ...
                <div>
                  {/* Rozet Vurgusu */}
                  <div className="inline-block rounded-md bg-blue-50 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-blue-700 ring-1 ring-inset ring-blue-700/10 transition-colors group-hover:bg-blue-600 group-hover:text-white">
                    Ayrıcalık
                  </div>

                  {/* Başlık */}
                  <h3 className="mt-5 text-xl font-bold tracking-tight text-slate-900 transition-colors group-hover:text-blue-700">
                    {item.title}
                  </h3>
                ...
                {/* Alt Kısım Mavi İlerleme Çizgisi */}
